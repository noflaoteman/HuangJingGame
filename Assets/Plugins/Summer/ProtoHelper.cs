...
namespace Summer
{
    ...
    public class ProtoHelper
    {
        ...
        static ProtoHelper()
        {
            ...
            IEnumerable<Type> q = from t in Assembly.GetExecutingAssembly().GetTypes() select t;
            q.ToList().ForEach(t =>
            {
                if (typeof(IMessage).IsAssignableFrom(t))
                {
                    MessageDescriptor desc = t.GetProperty("Descriptor").GetValue(t) as MessageDescriptor;
        ...
        {
            Type type = ProtoHelper.SeqType(typeCode);
            MessageDescriptor desc = type.GetProperty("Descriptor").GetValue(type) as MessageDescriptor;
            IMessage msg = desc.Parser.ParseFrom(data, offset, len);
