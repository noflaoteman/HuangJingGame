...
//namespace FlexiMotion.Editor
{
    ...
    public abstract class ReadMe : ScriptableObject
    {
        ...
        static ReadMe()
        {
            ...
            {
                foreach (IDisposable webRequest in TypeToUpdateCheck.Values)
                    webRequest.Dispose();
        ...
        private static void ShowReadMe()
        {
            ...
            {
                List<ReadMe> instances = FindInstances(out ReadMe autoSelect);
        ...
        {
            List<ReadMe> instances = new List<ReadMe>();

            DateTime latestWriteTime = default;
            autoSelect = null;
            string autoSelectGUID = null;

            string[] guids = AssetDatabase.FindAssets($"t:{nameof(ReadMe)}");
            for (int i = 0; i < guids.Length; i++)
            {
                string guid = guids[i];

                string assetPath = AssetDatabase.GUIDToAssetPath(guid);
                ReadMe asset = AssetDatabase.LoadAssetAtPath<ReadMe>(assetPath);
                if (asset == null)
                    continue;
                ...
                DateTime writeTime = File.GetLastWriteTimeUtc(assetPath);
        ...
        protected virtual void OnEnable()
        {
            ...
            string updateText = SessionState.GetString(name, "");
        ...
        public class Editor : UnityEditor.Editor
        {
            ...
            protected override void OnHeaderGUI()
            {
                GUILayout.BeginHorizontal(Styles.TitleArea);
                {
                    ...
                    float iconWidth = Styles.Title.CalcHeight(GUIContent, EditorGUIUtility.currentViewWidth);
            ...
            {
                string introduction = _Target.Introduction;
            ...
            private void DoNewVersionDetails()
            {
                ...
                string message = _Target._NewVersionAvailable
            ...
            private void DoCheckForUpdates()
            {
                ...
                Rect area = GUILayoutUtility.GetRect(0, EditorGUIUtility.singleLineHeight);
                area.xMin += EditorGUIUtility.singleLineHeight * 0.2f;

                EditorGUI.BeginChangeCheck();
                bool value = GUI.Toggle(area, _Target.CheckForUpdates, "Check For Updates");
            ...
            private void DoShowOnStartup()
            {
                ...
                GUIContent label = EditorGUI.BeginProperty(area, GUIContent, _DontShowOnStartupProperty);
                EditorGUI.BeginChangeCheck();
                bool value = _DontShowOnStartupProperty.boolValue;
            ...
            private void DoWarnings()
            {
                ...
                string directory = AssetDatabase.GetAssetPath(_Target);
                if (string.IsNullOrEmpty(directory))
                    return;

                directory = Path.GetDirectoryName(directory);

                string productName = _Target.ProductName;
            ...
            private void CheckDeleteDirectory(string directory)
            {
                ...
                string name = _Target.ProductName;
            ...
            protected virtual void DoExtraExamples()
            {
                ...
                for (int i = 0; i < _Target.ExtraExamples.Length; i++)
                {
                    ...
                    LinkSection section = _Target.ExtraExamples[i];
                    DoHeadingLink(
                        section.Heading,
                        section.Description,
                        section.URL,
            ...
            protected virtual void DoSupportBlock()
            {
                ...
                for (int i = 0; i < _Target.LinkSections.Length; i++)
                {
                    ...
                    LinkSection section = _Target.LinkSections[i];
                    DoHeadingLink(
                        section.Heading,
                        section.Description,
                        section.URL,
            ...
            {
                // Heading.
                GUIStyle style = url == null
                    ? Styles.HeaderLabel
                    : Styles.HeaderLink;
                Rect area = DoLinkButton(heading, url, style, fontSize);

                // Description.

                area.y += EditorGUIUtility.standardVerticalSpacing;

                int urlHeight = Styles.URL.fontSize + Styles.URL.margin.vertical;
            ...
            protected Rect DoLinkButton(string text, string url, GUIStyle style, int fontSize = 22)
            {
                ...
                Rect area = GUILayoutUtility.GetRect(0, size.y);

                Rect linkArea = new Rect(area.x, area.y, size.x, area.height);
                area.xMin += size.x;
            ...
            private class ExampleGroup
            {
                ...
                public static List<ExampleGroup> Gather(DefaultAsset rootDirectoryAsset, out string examplesDirectory)
                {
                    ...
                    string[] directories = Directory.GetDirectories(examplesDirectory);
                    List<ExampleGroup> groups = new List<ExampleGroup>();
                    bool allGroupsHaveOneScene = true;

                    for (int i = 0; i < directories.Length; i++)
                    {
                        ExampleGroup group = Gather(examplesDirectory, directories[i]);
                        if (group != null)
                        {
                            groups.Add(group);
                    ...
                    if (groups.Count == 0)
                    {
                        ExampleGroup group = Gather(examplesDirectory, examplesDirectory);
                        if (group != null)
                        {
                            groups.Add(group);
                            if (group.Scenes.Count > 1)
                ...
                public static ExampleGroup Gather(string rootDirectory, string directory)
                {
                    ...
                    {
                        SceneAsset scene = AssetDatabase.LoadAssetAtPath<SceneAsset>(files[j]);
                        if (scene != null)
                        {
                            if (scenes == null)
