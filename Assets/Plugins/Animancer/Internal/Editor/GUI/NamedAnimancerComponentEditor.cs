...
namespace Animancer.Editor
{
    ...
    public class NamedAnimancerComponentEditor : AnimancerComponentEditor
    {
        ...
        {
            Rect area = AnimancerGUI.LayoutSingleLineRect();

            float playAutomaticallyWidth = EditorGUIUtility.labelWidth + AnimancerGUI.ToggleWidth;
            Rect playAutomaticallyArea = AnimancerGUI.StealFromLeft(ref area, playAutomaticallyWidth);

            using (ObjectPool.Disposable.AcquireContent(out GUIContent label, playAutomatically))
                EditorGUI.PropertyField(playAutomaticallyArea, playAutomatically, label);

            SerializedProperty firstElement;
            AnimationClip clip;

            SerializedProperty animations = serializedObject.FindProperty("_Animations");
        ...
        private void DrawAnimationsHeader(Rect area)
        {
            ...
            SerializedProperty property = _Animations.serializedProperty;
            using (ObjectPool.Disposable.AcquireContent(out GUIContent label, property))
            {
        ...
        private void DrawAnimationElement(Rect area, int index, bool isActive, bool isFocused)
        {
            ...
            SerializedProperty element = _Animations.serializedProperty.GetArrayElementAtIndex(index);

            Color color = GUI.color;
            Object animation = element.objectReferenceValue;
