    ...
    public struct DampingJob : IAnimationJob
    {
        ...
        private void ComputeDampedPositions(AnimationStream stream)
        {
            ...
            for (int i = 1; i < jointHandles.Length; ++i)
            {
                ...
                Vector3 newPosition = parentPosition + (parentRotation * localPositions[i]);
        ...
        private void ComputeJointLocalRotations(AnimationStream stream)
        {
            ...
            for (int i = 0; i < jointHandles.Length - 1; ++i)
            {
                ...
                Quaternion currentToNewRotation = Quaternion.FromToRotation(direction, newDirection);
                ...
                Quaternion newLocalRotation = Quaternion.Inverse(parentRotation) * rotation;
