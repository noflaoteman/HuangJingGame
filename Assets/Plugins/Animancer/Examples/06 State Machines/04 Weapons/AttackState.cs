...
namespace Animancer.Examples.StateMachines
{
    ...
    public sealed class AttackState : CharacterState
    {
        ...
        private void OnEnable()
        {
            ...
            ClipTransition animation = Weapon.AttackAnimations[_AttackIndex];
        ...
        {
            ClipTransition[] attackAnimations = Weapon.AttackAnimations;

            if (_AttackIndex >= attackAnimations.Length - 1)
                return true;

            ClipState state = attackAnimations[_AttackIndex].State;
            if (state == null ||
                state.Weight == 0)
                return true;
