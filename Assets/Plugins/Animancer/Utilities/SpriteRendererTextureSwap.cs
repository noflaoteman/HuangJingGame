...
namespace Animancer
{
    ...
    public class SpriteRendererTextureSwap : MonoBehaviour
    {
        ...
        public static Dictionary<Sprite, Sprite> GetSpriteMap(Texture2D texture)
        {
            ...
            if (!TextureToSpriteMap.TryGetValue(texture, out Dictionary<Sprite, Sprite> map))
        ...
        public static bool TrySwapTexture(Dictionary<Sprite, Sprite> spriteMap, Texture2D texture, ref Sprite sprite)
        {
            ...
            if (!spriteMap.TryGetValue(sprite, out Sprite otherSprite))
            {
                ...
                string originalTextureName = sprite.texture.name;
                int index = name.IndexOf(originalTextureName);
                if (index >= 0)
                {
                    string newName =
                        texture.name +
        ...
        {
            if (TextureToSpriteMap.TryGetValue(texture, out Dictionary<Sprite, Sprite> spriteMap))
            {
