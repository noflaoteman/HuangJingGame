...
namespace Animancer
{
    ...
    public class DirectionalAnimationSet : ScriptableObject, IAnimationClipSource
    {
        ...
        public static Vector2 SnapVectorToDirection(Vector2 vector)
        {
            ...
            Direction direction = VectorToDirection(vector);
        ...
        public virtual int SetClipByName(AnimationClip clip)
        {
            ...
            int directionCount = ClipCount;
            for (int i = 0; i < directionCount; i++)
            {
                int index = name.LastIndexOf(GetDirectionName(i));
        ...
        {
            DirectionalAnimationSet set = (DirectionalAnimationSet)command.context;

            UnityEditor.Undo.RecordObject(set, "Find Animations");

            string directory = UnityEditor.AssetDatabase.GetAssetPath(set);
        ...
        {
            Dictionary<string, List<AnimationClip>> nameToAnimations = new Dictionary<string, List<AnimationClip>>();

            Object[] selection = UnityEditor.Selection.objects;
            for (int i = 0; i < selection.Length; i++)
            {
                AnimationClip clip = selection[i] as AnimationClip;
                if (clip == null)
                    continue;
                ...
                if (!nameToAnimations.TryGetValue(name, out List<AnimationClip> clips))
                {
            ...
            foreach (KeyValuePair<string, List<AnimationClip>> nameAndAnimations in nameToAnimations)
            {
                DirectionalAnimationSet set = nameAndAnimations.Value.Count <= 4 ?
                ...
                string path = UnityEditor.AssetDatabase.GetAssetPath(nameAndAnimations.Value[0]);
        ...
        private static void ToggleLooping(UnityEditor.MenuCommand command)
        {
            ...
            int count = set.ClipCount;
            for (int i = 0; i < count; i++)
            {
                AnimationClip clip = set.GetClip(i);
                if (clip == null)
                    continue;
