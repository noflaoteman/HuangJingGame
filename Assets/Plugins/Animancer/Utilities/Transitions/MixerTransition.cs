...
namespace Animancer
{
    ...
    public class MixerTransitionDrawer : ManualMixerTransition.Drawer
    {
        ...
        protected override void GatherSubProperties(SerializedProperty property)
        {
            ...
            int count = Math.Max(CurrentAnimations.arraySize, CurrentThresholds.arraySize);
            ...
            out Rect animation, out Rect threshold, out Rect speed, out Rect sync)
        {
            ...
            else
            {
                ...
                float xMin = threshold.xMin = EditorGUIUtility.labelWidth + AnimancerGUI.IndentSize;
        ...
        {
            SplitListRect(area, true, out Rect animationArea, out Rect thresholdArea, out Rect speedArea, out Rect syncArea);

            DoAnimationHeaderGUI(animationArea);

            ThresholdLabelAttribute attribute = AttributeCache<ThresholdLabelAttribute>.FindAttribute(CurrentThresholds);
            string text = attribute != null
                ? attribute.Label
                : "Threshold";

            using (ObjectPool.Disposable.AcquireContent(out GUIContent label, text,
            ...
            SerializedProperty animation, SerializedProperty speed)
        {
            SplitListRect(area, false,
                out Rect animationArea, out Rect thresholdArea, out Rect speedArea, out Rect syncArea);
        ...
        {
            SerializedProperty threshold = CurrentThresholds.GetArrayElementAtIndex(index);
