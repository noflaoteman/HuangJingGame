        ...
        ManualMixerState.ITransition, ICopyable<ManualMixerTransition>
    {
        ...
        public class Drawer : TransitionDrawer
        {
            ...
            protected virtual ReorderableList GatherDetails(SerializedProperty property)
            {
                ...
                if (!PropertyPathToStates.TryGetValue(path, out ReorderableList states))
                {
            ...
            public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
            {
                ...
                {
                    ReorderableList states = GatherDetails(property);
                    if (states != null)
                ...
                out Rect animation, out Rect speed, out Rect sync)
            {
                ...
                float spacing = StandardSpacing;

                float syncWidth = isHeader ?
                    _SyncLabelWidth :
                    ToggleWidth - spacing;

                float speedWidth = _SpeedLabelWidth + _SyncLabelWidth - syncWidth;
                if (!isHeader)
                {
                    // Don't use Clamp because the max might be smaller than the min.
                    float max = Math.Max(area.height, area.width * 0.25f - 30);
            ...
            {
                SplitListRect(area, true, out Rect animationArea, out Rect speedArea, out Rect syncArea);
            ...
            {
                using (ObjectPool.Disposable.AcquireContent(out GUIContent label, "Animation",
            ...
            {
                using (ObjectPool.Disposable.AcquireContent(out GUIContent label, "Speed", Strings.Tooltips.Speed))
                {
            ...
            {
                int speedCount = CurrentSpeeds.arraySize;

                float[] lengths = new float[CurrentAnimations.arraySize];
                if (lengths.Length <= 1)
                    return;
                ...
                {
                    Object state = CurrentAnimations.GetArrayElementAtIndex(i).objectReferenceValue;
                    if (AnimancerUtilities.TryGetLength(state, out float length) &&
                        length > 0)
                    {
                        nonZeroLengths++;
                        totalLength += length;
                ...
                float averageLength = totalLength / nonZeroLengths;
                float averageSpeed = speedCount > 0 ? totalSpeed / nonZeroLengths : 1;
            ...
            {
                int count = CurrentSpeeds.arraySize;
                while (start < count)
            ...
            {
                using (ObjectPool.Disposable.AcquireContent(out GUIContent label, "Sync",
                    "Determines which child states have their normalized times constantly synchronized"))
                {
                    DoHeaderDropdownGUI(area, CurrentSpeeds, label, menu =>
                    {
                        int syncCount = CurrentSynchronizeChildren.arraySize;

                        MenuFunctionState allState = syncCount == 0 ? MenuFunctionState.Selected : MenuFunctionState.Normal;
                        AddPropertyModifierFunction(menu, "All", allState,
                            (_) => CurrentSynchronizeChildren.arraySize = 0);

                        bool syncNone = syncCount == CurrentAnimations.arraySize;
                        if (syncNone)
                        {
                        ...
                        MenuFunctionState noneState = syncNone ? MenuFunctionState.Selected : MenuFunctionState.Normal;
                        AddPropertyModifierFunction(menu, "None", noneState, (_) =>
                        {
                            int count = CurrentSynchronizeChildren.arraySize = CurrentAnimations.arraySize;
                        ...
                        {
                            int count = CurrentSynchronizeChildren.arraySize;
                            for (int i = 0; i < count; i++)
                            {
                                SerializedProperty property = CurrentSynchronizeChildren.GetArrayElementAtIndex(i);
                                property.boolValue = !property.boolValue;
                            }

                            int newCount = CurrentSynchronizeChildren.arraySize = CurrentAnimations.arraySize;
                        ...
                        {
                            int count = CurrentAnimations.arraySize;
                            ...
                            for (int i = 0; i < count; i++)
                            {
                                ...
                                CurrentSynchronizeChildren.GetArrayElementAtIndex(i).boolValue =
                                    AnimancerUtilities.TryGetAverageVelocity(state, out Vector3 velocity) &&
                                    velocity != default;
                            }
            ...
            protected virtual void DoChildListFooterGUI(Rect area)
            {
                ...
                using (ObjectPool.Disposable.AcquireContent(out GUIContent label, "Count"))
                {
                    int indentLevel = EditorGUI.indentLevel;
                    EditorGUI.indentLevel = 0;
                    ...
                    int count = EditorGUI.DelayedIntField(area, label, _CurrentChildList.count);
            ...
            private void DoElementGUI(Rect area, int index, bool isActive, bool isFocused)
            {
                ...
                SerializedProperty state = CurrentAnimations.GetArrayElementAtIndex(index);
                SerializedProperty speed = CurrentSpeeds.arraySize > 0
            ...
            {
                SplitListRect(area, false, out Rect animationArea, out Rect speedArea, out Rect syncArea);
            ...
            public static void DoAnimationField(Rect area, SerializedProperty property)
            {
                ...
                bool allowSceneObjects = targetObject != null && !EditorUtility.IsPersistent(targetObject);

                EditorGUI.BeginChangeCheck();
                Object newReference = EditorGUI.ObjectField(area, GUIContent.none, oldReference, type, allowSceneObjects);
                if (EditorGUI.EndChangeCheck())
                {
            ...
            protected void DoSpeedFieldGUI(Rect area, SerializedProperty speed, int index)
            {
                ...
                else// If this element doesn't have its own speed property, just show 1.
                {
                    ...
                    float value = Units.UnitsAttribute.DoSpecialFloatField(
            ...
            {
                SerializedProperty syncProperty = CurrentSynchronizeChildren;
                int syncFlagCount = syncProperty.arraySize;
            ...
            {
                SerializedProperty property = CurrentSpeeds;
                if (property == null)
                    return;

                int speedCount = property.arraySize;
                if (speedCount <= 0)
                    return;
            ...
            {
                SerializedProperty property = CurrentSynchronizeChildren;
                if (property == null)
                    return;
