        ...
        PlayableAssetState.ITransition, IAnimationClipCollection, ICopyable<PlayableAssetTransition>
    {
        ...
        public class Drawer : TransitionDrawer
        {
            ...
            public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
            {
                ...
                {
                    SerializedProperty bindings = property.FindPropertyRelative(nameof(_Bindings));
                    if (bindings != null)
                    {
            ...
            {
                int outputCount = GetOutputCount(out IEnumerator<PlayableBinding> outputEnumerator, out bool firstBindingIsAnimation);
                ...
                property.Next(true);
                // Array Size.
                DoBindingsCountGUI(area, property, label, outputCount, firstBindingIsAnimation, out int bindingCount);
            ...
            private int GetOutputCount(out IEnumerator<PlayableBinding> outputEnumerator, out bool firstBindingIsAnimation)
            {
                ...
                {
                    IEnumerable<PlayableBinding> outputs = _CurrentAsset.outputs;
                    _CurrentAsset = null;
                    ...
                    while (outputEnumerator.MoveNext())
                    {
                        PlayableAssetState.GetBindingDetails(
                            outputEnumerator.Current, out string _, out Type _, out bool isMarkers);
                        if (isMarkers)
                            continue;
                ...
                int outputCount, bool firstBindingIsAnimation, out int bindingCount)
            {
                ...
                if (bindingCount != outputCount && !(bindingCount == 0 && outputCount == 1 && firstBindingIsAnimation))
                {
                    ...
                    GUIStyle style = AnimancerGUI.MiniButton;

                    string countLabel = outputCount.ToString();
                    float fixSizeWidth = AnimancerGUI.CalculateWidth(style, countLabel);
                    Rect fixSizeArea = AnimancerGUI.StealFromRight(
                ...
                IEnumerator<PlayableBinding> outputEnumerator, int trackIndex)
            {
                CheckIfSkip:
                PlayableAssetState.GetBindingDetails(
                    outputEnumerator.Current, out string name, out Type bindingType, out bool isMarkers);
                ...
                ref Object obj, string tooltip)
            {
                ...
                GUI.color = AnimancerGUI.WarningFieldColor;
                GUIStyle miniButton = AnimancerGUI.MiniButton;

                string text = label.text;
                label.text = "x";

                float xWidth = AnimancerGUI.CalculateWidth(miniButton, label);
                Rect xArea = AnimancerGUI.StealFromRight(
