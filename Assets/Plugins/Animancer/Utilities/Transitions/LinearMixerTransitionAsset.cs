...
namespace Animancer
{
        ...
        LinearMixerState.ITransition, ICopyable<LinearMixerTransition>
    {
        ...
        {
            int thresholdCount = Thresholds.Length;
            if (thresholdCount <= 1)
                return;

            int speedCount = Speeds.Length;
            int syncCount = SynchronizeChildren.Length;

            float previousThreshold = Thresholds[0];
        ...
        public class Drawer : MixerTransitionDrawer
        {
            ...
            protected override void DoThresholdGUI(Rect area, int index)
            {
                ...
                if (index > 0)
                {
                    UnityEditor.SerializedProperty previousThreshold = CurrentThresholds.GetArrayElementAtIndex(index - 1);
                    UnityEditor.SerializedProperty currentThreshold = CurrentThresholds.GetArrayElementAtIndex(index);
                    if (previousThreshold.floatValue >= currentThreshold.floatValue)
                    {
                        ...
                        Rect iconArea = Editor.AnimancerGUI.StealFromRight(ref area, area.height, Editor.AnimancerGUI.StandardSpacing);
            ...
            protected override void AddThresholdFunctionsToMenu(UnityEditor.GenericMenu menu)
            {
                ...
                int count = CurrentThresholds.arraySize;
                if (count <= 1)
                {
                ...
                {
                    float first = CurrentThresholds.GetArrayElementAtIndex(0).floatValue;
                    float last = CurrentThresholds.GetArrayElementAtIndex(count - 1).floatValue;
                ...
                AddCalculateThresholdsFunction(menu, "From Speed",
                    (state, threshold) => AnimancerUtilities.TryGetAverageVelocity(state, out Vector3 velocity) ? velocity.magnitude : float.NaN);
                AddCalculateThresholdsFunction(menu, "From Velocity X",
                    (state, threshold) => AnimancerUtilities.TryGetAverageVelocity(state, out Vector3 velocity) ? velocity.x : float.NaN);
                AddCalculateThresholdsFunction(menu, "From Velocity Y",
                    (state, threshold) => AnimancerUtilities.TryGetAverageVelocity(state, out Vector3 velocity) ? velocity.y : float.NaN);
                AddCalculateThresholdsFunction(menu, "From Velocity Z",
                    (state, threshold) => AnimancerUtilities.TryGetAverageVelocity(state, out Vector3 velocity) ? velocity.z : float.NaN);
                AddCalculateThresholdsFunction(menu, "From Angular Speed (Rad)",
                    (state, threshold) => AnimancerUtilities.TryGetAverageAngularSpeed(state, out float speed) ? speed : float.NaN);
                AddCalculateThresholdsFunction(menu, "From Angular Speed (Deg)",
                    (state, threshold) => AnimancerUtilities.TryGetAverageAngularSpeed(state, out float speed) ? speed * Mathf.Rad2Deg : float.NaN);
            }
                ...
                Func<Object, float, float> calculateThreshold)
            {
                AddPropertyModifierFunction(menu, label, (property) =>
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        UnityEditor.SerializedProperty threshold = CurrentThresholds.GetArrayElementAtIndex(i);
                        float value = calculateThreshold(state, threshold.floatValue);
