        ...
        where TMixer : ManualMixerState
    {
        ...
        public override bool IsLooping
        {
            get
            {
                ...
                {
                    if (AnimancerUtilities.TryGetIsLooping(_Animations[i], out bool isLooping) &&
                        isLooping)
                        return true;
                }
        ...
        public override float MaximumDuration
        {
            get
            {
                ...
                for (int i = _Animations.Length - 1; i >= 0; i--)
                {
                    if (!AnimancerUtilities.TryGetLength(_Animations[i], out float length))
                        continue;
        ...
        public virtual float AverageAngularSpeed
        {
            get
            {
                ...
                for (int i = _Animations.Length - 1; i >= 0; i--)
                {
                    if (AnimancerUtilities.TryGetAverageAngularSpeed(_Animations[i], out float speed))
                    {
                        if (hasSpeeds)
                            speed *= _Speeds[i];
        ...
        public virtual Vector3 AverageVelocity
        {
            get
            {
                ...
                for (int i = _Animations.Length - 1; i >= 0; i--)
                {
                    if (AnimancerUtilities.TryGetAverageVelocity(_Animations[i], out Vector3 velocity))
                    {
                        if (hasSpeeds)
                            velocity *= _Speeds[i];
        ...
        public virtual void InitializeState()
        {
            ...
            bool auto = ManualMixerState.SynchronizeNewChildren;
            try
            {
            ...
            if (_Speeds != null)
            {
                ...
                int count = Math.Min(_Animations.Length, _Speeds.Length);
